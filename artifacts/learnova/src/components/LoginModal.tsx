...
export function LoginModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setError('Email/password login is not enabled in Firebase Console. An admin must enable it in Firebase Authentication settings.');
      } else {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        setError('Google login is not enabled in Firebase Console. An admin must enable it in Firebase Authentication settings.');
      } else {
        setError(err.message || 'Authentication failed');
      }
    } finally {
